...
module.exports = Router()
  ...
  // DELETE AN INDIVIDUAL CHANNEL BY CHANNEL ID
  .delete('/:channel_id', async (req, res, next) => {
    try {
      await Channel.deleteByChannelId(req.params.channel_id);
      res.send({ message: 'Successfully deleted channel' });
    } catch (error) {
      next(error);
    }
  });
